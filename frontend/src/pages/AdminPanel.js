...
function AdminPanel() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setStats({
                ...
                activeUsers: usersRes.data.filter(user => user.lastActive && 
                    new Date(user.lastActive) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length,
                completionRate: Math.round(
                    usersRes.data.reduce((acc, user) => 
                        acc + (user.progress?.reduce((sum, p) => sum + p.completion, 0) || 0), 0) / 
        ...
        try {
            const res = await axios.put(
                ...
                { 
                    ...
                    progress: [{ completion: parseFloat(editProgress) }] 
                },
                { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    const exportData = () => {
        const data = activeTab === 'users' ? users : courses;
        ...
        <div className="admin-panel">
            <div className="admin-header">
                <div className="header-content">
                    <h1><FaGraduationCap className="header-icon" />Admin Dashboard</h1>
            ...
            <div className="tab-content">
                ...
                {activeTab === 'courses' && (
                    <div className="courses-section">
                        <div className="section-header">
                            <h2><FaBook />Course Management</h2>
                            <div className="header-actions">
                                <div className="search-box">
                                    <FaSearch className="search-icon" />
                                    <input
